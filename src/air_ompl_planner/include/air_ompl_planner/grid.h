...
public:
  ...
  Eigen::Vector3i Pos2SubExtend(const Eigen::Vector3d& pos, const float& margin_d, Eigen::Vector3i& flag_vec)
  {
    Eigen::Vector3i sub = Pos2Sub(pos);
    for (int i = 0; i < dimension_; i++)
    {
      flag_vec(i) = 0;
      if (sub(i) >= 0 && sub(i) < size_(i)) {
        const double d = (pos(i) - origin_(i)) * resolution_inv_(i) - sub(i);
        if (d * resolution_(i) < margin_d) {
          flag_vec(i) = -1;
        } else if (d * resolution_(i) > resolution_(i) - margin_d) {
          flag_vec(i) = 1;
        }
      }
    }
