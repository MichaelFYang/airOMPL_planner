...
#include "air_ompl_planner/scan_handler.h"
...
void ScanHandler::UpdateRobotPosition(const Point3D& robot_pos) {
    ...
    center_p_.x = robot_pos.x, center_p_.y = robot_pos.y, center_p_.z = robot_pos.z;
}
...
void ScanHandler::SetCurrentScanCloud(const PointCloudPtr& scanCloudIn) {
    ...
    for (const auto& point : copyObsScanCloud->points) { // assign obstacle scan voxels
        ...
        const int L = static_cast<int>(std::ceil((r * ANG_RES_X)/scan_params_.voxel_size/2.0f)) + 2; // inflate size: 2
...
void ScanHandler::SetSurroundObsCloud(const PointCloudPtr& obsCloudIn, const bool& is_filter_cloud) {
    ...
    if (is_filter_cloud) AOMPLUtil::FilterCloud(obsCloudIn, scan_params_.voxel_size);
...
void ScanHandler::SetRayCloud(const Eigen::Vector3i& point_sub) {
    ...
    sign_sub.x() = AOMPLUtil::Signum(dir_sub.x()), sign_sub.y() = AOMPLUtil::Signum(dir_sub.y()), sign_sub.z() = AOMPLUtil::Signum(dir_sub.z());
